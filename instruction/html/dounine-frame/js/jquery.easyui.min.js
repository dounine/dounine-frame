...
(function($) {
	...
	function _179(_17a, id) {
		/* dounine-tag-bug */
		if(arguments[2]){
			return _11d(_17a, arguments[2], id);
		}
	...
	$.fn.tree = function(_192, _193) {
		if (typeof _192 == "string") {
			/* dounine-tag-bug */
			return $.fn.tree.methods[_192](this, _193, arguments[2]);
		}
		var _192 = _192 || {};
		return this.each(function() {
	...
	$.fn.tree.methods = {
		...
		find : function(jq, id) {
			/* dounine-tag-bug */
			return _179(jq[0], id, arguments[2]);
		},
		...
		render : function(_1b5, ul, data) {
			...
			function _1b7(_1b8, _1b9) {
				...
				for (var i = 0; i < _1b9.length; i++) {
					...
					item.textField = opts.textField;
					item.valueField = opts.valueField;
					/** dounine-tag-bug **/
	...
	$.fn.tree.defaults = {
		...
		loader : function(_1bb, _1bc, _1bd) {
			...
			$.ajax({
				...
				url : opts.url,
				data : _1bb,
				//dataType : "html",//dounine update data(json)
				success : function(data) {
					_1bc(dataFilter(data,'json'));
				},
				error : function() {
					dataFilter(arguments);
	...
	function load(_4c0, data) {
		...
		if (typeof data == "string") {
			...
			$.ajax({
				url : data,
				data : _4c1,
				//dataType : "html",//dounine update data
				success : function(data) {
					_4c2(dataFilter(data,'json'));
				},
				error : function() {
					dataFilter(arguments);
...
(function($) {
	...
	function _971(_972) {
		...
		if (opts.multiple) {
			...
			for (var i = 0; i < _974.length; i++) {
				if (_974[i].valueField == null){
					vv.push(_974[i].id);
				}else{
					vv.push(_974[i][_974[i].valueField]);
				}
				if (_974[i].textField == null) {
					/** dounine-tag-bug * */
					ss.push(_974[i].text);
				} else {
					ss.push(_974[i][_974[i].textField]);
				}
			}
		} else {
			var node = tree.tree("getSelected");
			if (node) {
				if (node.valueField == null) {
					/** dounine-tag-bug * */
					vv.push(node.id);
				} else {
					vv.push(node[node.valueField]);
				}
				if (node.textField == null) {
					/** dounine-tag-bug * */
					ss.push(node.text);
				} else {
					ss.push(node[node.textField]);
				}
			}
		}
	...
	function _975(_976, _977) {
		...
		for (var i = 0; i < _977.length; i++) {
			/* dounine-tag-bug */
			var node = tree.tree("find", _977[i] , opts.valueField);
			if (node) {
